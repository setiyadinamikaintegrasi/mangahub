...
|---|---|---|
| `detect-stack.sh` | Called by `Makefile` and CI automatically; run manually to debug | Prints the detected stack token (`python \| node \| go \| java \| dotnet \| unknown`) for single-stack layouts. A version-2 monorepo returns `unknown` because the dispatcher resolves its explicit components. Always exits 0. |
...
| `resolve-components.sh` | Component-aware CI | Validates and emits the explicit version-2 component list as TSV or JSON; it never discovers manifests. |
